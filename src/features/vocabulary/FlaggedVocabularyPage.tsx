...
export function FlaggedVocabularyPage({ flag }: { flag: 'favorite' | 'difficult' }) {
  ...
  return <section>
    <header className="page-header"><h1>{title}</h1><p>{flag === 'favorite' ? 'Words saved for quick access.' : 'Words that deserve extra attention.'}</p></header>
    {error && <p className="notice error">{error}</p>}
    <div className="table-wrap flagged-table">
      <table><thead><tr><th>Word</th><th>Meaning</th><th>Status</th><th>Priority</th><th>Action</th></tr></thead>
        <tbody>{words.map((word) => <tr key={word.id}>
          <td><Link to={`/vocabulary/${word.id}`}>{word.lemma}</Link></td>
          <td>{word.definitionZh || word.definitionEn || '—'}</td><td>{word.status}</td><td>{word.priorityScore}</td>
          <td><button className="text-button" onClick={() => void remove(word.id)}>Remove</button></td>
        </tr>)}</tbody>
      </table>
      {!loading && words.length === 0 && <div className="empty">No {title.toLowerCase()} words yet.</div>}
